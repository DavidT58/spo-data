...
func (m *Monitor) checkBlocks(ctx context.Context, firstPass bool) {
	...
	for _, pool := range m.cfg.Pools {
		...
		if firstPass && !a.monitorable {
			m.logger.Printf("info: %s (%s) produces ~%.2f blocks/epoch — too few to monitor reliably by block production (threshold capped at %s)",
				pool.Name, pool.Operator, a.expBlocks, fmtDuration(a.threshold))
		}
